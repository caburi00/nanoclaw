...
export async function requestApproval(opts: RequestApprovalOptions): Promise<void> {
  ...
  if (adapter) {
    try {
      ...
      // Record where the card landed so the channel adapter can rehydrate its
      // in-memory pending-card map after a restart (otherwise this approval
      // could never be answered post-restart).
